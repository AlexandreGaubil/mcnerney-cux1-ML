...
if (!dir.exists("./data/output/")) {
  warning("No `data/output/` directory found, creating one now...")
  dir.create("./data/output/")
}
if (!dir.exists("./data/input/")) {
  stop("No `data/input/` directory found.")
}
if (!dir.exists("./data/cache/")) {
  warning("No `data/cache/` directory found, creating one now...")
...
```{r}
# Define type of data transformation to apply, depending on the dataset we are working with
settings.dataset <- "in_vitro"
# settings.dataset <- "in_vivo"

# If TRUE, will remove variables as they are no longer needed. Keep FALSE while developing to facilitate debugging
settings.remove_unused <- TRUE
```
...
if (!file.exists("./data/cache/input_for_Seurat/matrix.mtx.gz")) {
  print("Creating `matrix.mtx.gz`...")
  # Load the L1 normalized count matrix
  count_mtx = readMM("./data/input/normed_counts.mtx")

  # Read10X() expect gene-by-cell-matrix, so we need to transpose and export
  count_mtx <- t(count_mtx)
  if (!dir.exists("./data/cache/input_for_Seurat/")) {
    dir.create("./data/cache/input_for_Seurat/")
  }
  file.create("./data/cache/input_for_Seurat/matrix.mtx")
  writeMM(count_mtx, "./data/cache/input_for_Seurat/matrix.mtx")
  gzip("./data/cache/input_for_Seurat/matrix.mtx")
  file.remove("./data/cache/input_for_Seurat/matrix.mtx")
}
```

## Load `metadata.txt`
```{r}
meta_data <- read.delim("./data/input/metadata.txt", header = TRUE)
```
...
```{r}
if (!file.exists("./data/cache/t_clone_matrix.mtx")) {
  print("Creating `t_clone_matrix.mtx`...")
  clone_matrix <- readMM("./data/input/clone_matrix.mtx")
  t_clone_matrix <- t(clone_matrix)

  file.create("./data/cache/t_clone_matrix.mtx")
  writeMM(t_clone_matrix, "./data/cache/t_clone_matrix.mtx")
...
t_clone_matrix <- readMM("./data/cache/t_clone_matrix.mtx")
...
```{r}
if (!file.exists("./data/cache/input_for_Seurat/barcodes.tsv.gz")) {
  print("Creating `barcode.tsv.gz`...")
  barcode <- as.data.frame(rownames(meta_data), col.names = NULL)

  # Export the new cell barcode tsv file
  write.table(barcode, file='./data/cache/input_for_Seurat/barcodes.tsv', quote=FALSE, sep='\t', col.names=FALSE, row.names=FALSE)
  gzip("./data/cache/input_for_Seurat/barcodes.tsv")
  file.remove("./data/cache/input_for_Seurat/barcodes.tsv")
}
```

## Read in the Seurat object
```{r}
LT.data <- Read10X(data.dir="./data/cache/input_for_Seurat", gene.column=1, cell.column = 1)
...
saveRDS(LT.seu.day2, file = "./data/cache/LT_seu_day2_before_filtering.rds")

# Load LT.seu.day2 from file
LT.seu.day2 <- readRDS(file = "./data/cache/LT_seu_day2_before_filtering.rds")
```
...
if (settings.dataset == "in_vitro") {
...
} else if (settings.dataset == "in_vivo") {
  LT.seu.day2@meta.data <- LT.seu.day2@meta.data %>%
  ...
  stop("Unrecognized value for `settings.dataset`")
}
...
```{r}
saveRDS(LT.seu.day2, file = "./data/cache/LT_seu_day2_ML_source_data.rds")
```
...
write.table(day2.count.neg.ctrl.data,
            row.names = FALSE,
            col.names = FALSE,
            "./data/output/neg_control/rand_genes_neg_ctrl_data_all_cell_types.txt")
write.table(day2.count.neg.ctrl.feature,
            row.names = FALSE,
            col.names = FALSE,
            "./data/output/neg_control/rand_genes_neg_ctrl_feature_all_cell_types.txt")
```
...
#export the df
write.table(day2.count.neg.ctrl,"./data/output/ML_classifier/pos_control/var_3000_pos_ctrl.txt")
```
