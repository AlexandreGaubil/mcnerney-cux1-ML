...
if (!dir.exists("./data/")) {
  print("It seems that the current working directory is not correct, you should check that.")
  stop()
}
```
...
if (!file.exists("./data/input_matrix_for_Seurat/matrix.mtx")) {
...
```{r}
if (!file.exists("./data/output/t_clone_matrix.mtx")) {
  clone_matrix <- readMM("./data/clone_matrix.mtx")
  t_clone_matrix <- t(clone_matrix)
  if (!dir.exists("./output/")) {
    dir.create("./data/output/")
  }
  file.create("./data/output/t_clone_matrix.mtx")
...
t_clone_meta_data_df <- t(clone_meta_data_df)
clone_membership <- lapply(t_clone_meta_data_df, function(x) match(1,x))
rm(t_clone_meta_data_df)
# clone_membership
...
```{r}
library(stringr)
#create a new cell barcode column which incorporate library ID
...
```{r}
if (!file.exists("./data/barcodes_unique.tsv")) {
  barcode <- as.data.frame(rownames(meta_data), col.names = NULL)
  
  # Export the new cell barcode tsv file
  write.table(barcode, file='./data/barcodes_unique.tsv', quote=FALSE, sep='\t', col.names=FALSE, row.names=FALSE)
}
```

Now read in the Seurat object
```{r}
LT.data <- Read10X(data.dir="./data/input_matrix_for_Seurat", gene.column=1)
